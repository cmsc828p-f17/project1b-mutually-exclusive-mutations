...
def weight(solution): # since this is all in main, not sure if I need thees inputs really
    lambdaG = 0
    lambdaM = 0
    examples_seen = set()
...
# meaning we can test more potential gene sets?
        ...
        # compute exponent
        # NOTE: HERE, AND VANDIN ET AL CODE, THEY DO NOT MULTIPLY BY C=0.5
   ...
   # in your comment, you wanted to report the distribution of weights
   # I report only the number of iterations with a set with a greater weight
   # this is done for memory/I don't know if you care about which sets had greater weight
   # it could be useful though, so please let me know if you want me to change this
   # it would be a pretty easy change
    return num_greaterweight
...
# but I keep it since this is how a formulate the gene_Setexamples set, so I have to keep it for now
    ...
    for runs in range(numruns):
        ...
        #### permutating the given mutation matrix
        ...
        #### running mcmc function, outputs number of iterations where the stat was found
        num_greaterweight = mcmc(testweight)
